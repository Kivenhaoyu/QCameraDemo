 ...
 *  @param blockError 失败回掉
 */
+(void)hTTPRequest_GetpostBody:(NSDictionary *)body andUrl:(NSString *)url andSucceed:(Succeed)succeed andFailure:(Failure)failure andISstatus:(BOOL)isStatus andHeader:(NSArray *)headerArr
{
    ...
    [httpRequestPost setconManagefiguration];
    for (int i = 0; i<headerArr.count/2; i++) {
        [httpRequestPost.AFAppDotNetClient.requestSerializer setValue:headerArr[i*2+1] forHTTPHeaderField:headerArr[i*2]];
    }
 ...
 *  @param blockError 失败回掉
 */
+(void)hTTPRequest_PUTpostBody:(NSDictionary *)body andUrl:(NSString *)url andSucceed:(Succeed)succeed andFailure:(Failure)failure andISstatus:(BOOL)isStatus andHeader:(NSArray *)headerArr
{
    DebugLog(@"%@",body);
...
-(void)setconManagefiguration
{
    ...
    if ([UserInfoClass sheardUserInfo].isLogin) {
        [self.AFAppDotNetClient.requestSerializer setValue:[UserInfoClass sheardUserInfo].author_token forHTTPHeaderField:@"Authorization"];
    }
    if ([UserInfoClass sheardUserInfo].bucket) {
        [self.AFAppDotNetClient.requestSerializer setValue:[UserInfoClass sheardUserInfo].bucket forHTTPHeaderField:@"X-Qiniu-Bucket"];
    }
